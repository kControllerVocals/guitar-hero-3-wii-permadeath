...
create_match_fs = {
	...
	actions = [
		{
			action = select_done
			flow_state_func = friends_or_strangers_create_match
		}
		{
			action = go_back
			flow_state = online_menu_fs
...
script friends_or_strangers_create_match 
	setup_and_post_game
...
create_match_select_venue_fs = {
	...
	actions = [
		...
		{
			action = continue
			flow_state_func = select_venue_flow_state_func
		}
		{
			action = go_back
			flow_state = create_match_select_num_songs_fs
...
script setup_and_invite_friend 
	change \{choose_specific_friend = 1}
endscript

script select_venue_flow_state_func 
	if ($who_can_join = friends)
		setup_and_invite_friend
		return \{flow_state = online_friendlist_fs}
	else
		setup_and_post_game
		return \{flow_state = online_character_select_fs}
	endif
