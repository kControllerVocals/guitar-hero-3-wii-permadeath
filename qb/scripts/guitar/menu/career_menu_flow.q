...
career_sponsored_fs = {
	...
	actions = [
		{
			action = continue
			flow_state = career_movie_fs
		}
	]
}

script delayed_continue_or_some_crap 
...
endscript
career_movie_fs = {
	create = delayed_continue_or_some_crap
	actions = [
		{
			action = continue
			flow_state_func = career_autosave_or_setlist
...
career_battle_help_fs = {
	...
	actions = [
		{
			action = continue
			func = unload_pak_then_start_song
			transition_screen = default_loading_screen
			flow_state = career_play_song_fs
		}
		{
...
script unload_pak_then_start_song 
...
script prepare_bassist_for_encore 
	...
	get_song_rhythm_track_cfunc song = <song>
	if (<rhythm_track> = 1)
...
career_battle_help_boss_confirmation_fs = {
	...
	actions = [
		{
			action = continue
			func = unload_screen_pak_then_start_boss
			flow_state = career_play_song_fs
		}
		{
			action = go_back
...
script unload_screen_pak_then_start_boss 
	SetPakManCurrentBlock \{map = flow_state_pak_map
		pak = none
		block_scripts = 1}
	start_boss
