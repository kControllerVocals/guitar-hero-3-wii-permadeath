...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_career || $game_mode = p2_coop || $game_mode = p1_quickplay || <for_practice> = 1)
		...
		if find_profile_by_id id = ($<player_status>.character_id)
			...
			FormatText checksumname = ss_character_photo 'Mag_Photo_Guitarist_%n_Outfit%o_Style%s' n = <character_name> o = <outfit> s = <style>
		endif
		displaySprite {
			parent = newspaper_container
			tex = <ss_character_photo>
			dims = (640.0, 640.0)
			pos = (125.0, 34.0)
			z = -1
		}
		...
		switch $g_ss_mag_number
			...
			<ss_percent_fill_dims> = (350.0, 168.0)
			<ss_percent_fill_pos> = (262.0, 460.0)
			<ss_percent_color> = $g_ss_black
			...
			<ss_percent_fill_dims> = (376.0, 178.0)
			<ss_percent_fill_pos> = (262.0, 455.0)
			<ss_percent_color> = $g_ss_black
			<ss_star_good_color> = $g_ss_offwhite
			<ss_star_bad_color> = [100 100 100 255]
			<ss_score_fill_L_color> = $g_ss_black
			...
			<ss_percent_fill_dims> = (376.0, 158.0)
			<ss_percent_fill_pos> = (272.0, 457.0)
			...
			<ss_percent_fill_dims> = (385.0, 198.0)
			<ss_percent_fill_pos> = (272.0, 442.0)
			<ss_percent_color> = $g_ss_decibel_magentaish
			...
			<ss_percent_fill_dims> = (414.0, 188.0)
			<ss_percent_fill_pos> = (252.0, 447.0)
			...
			<ss_percent_fill_dims> = (396.0, 178.0)
			<ss_percent_fill_pos> = (262.0, 455.0)
			...
			<ss_percent_fill_dims> = (396.0, 215.0)
			<ss_percent_fill_pos> = (262.0, 430.0)
			...
			<ss_percent_fill_dims> = (366.0, 178.0)
			<ss_percent_fill_pos> = (262.0, 445.0)
		...
		displaySprite {
			parent = newspaper_container
			tex = <ss_percent_fill>
			pos = <ss_percent_fill_pos>
			dims = <ss_percent_fill_dims>
		}
		FormatText TextName = p1_notes_hit '%d' d = <p1_percent_complete>
		if (<p1_percent_complete> = 100)
